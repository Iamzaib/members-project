...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.family.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.families.update", [$family->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ledenid') ? 'is-invalid' : '' }}" name="ledenid_id" id="ledenid_id">
                    @foreach($ledenids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ledenid_id') ? old('ledenid_id') : $family->ledenid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('family_member_type') ? 'is-invalid' : '' }}" name="family_member_type" id="family_member_type" required>
                    ...
                    @foreach(App\Models\Family::FAMILY_MEMBER_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('family_member_type', $family->family_member_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('first_name') ? 'is-invalid' : '' }}" type="text" name="first_name" id="first_name" value="{{ old('first_name', $family->first_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('surname') ? 'is-invalid' : '' }}" type="text" name="surname" id="surname" value="{{ old('surname', $family->surname) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('d_o_b') ? 'is-invalid' : '' }}" type="text" name="d_o_b" id="d_o_b" value="{{ old('d_o_b', $family->d_o_b) }}">
                @if($errors->has('d_o_b'))
                    <div class="invalid-feedback">
                        {{ $errors->first('d_o_b') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Family::GENDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('gender') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="gender_{{ $key }}" name="gender" value="{{ $key }}" {{ old('gender', $family->gender) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Family::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $family->status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('birthplace') ? 'is-invalid' : '' }}" type="text" name="birthplace" id="birthplace" value="{{ old('birthplace', $family->birthplace) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('history_almere') ? 'is-invalid' : '' }}" name="history_almere" id="history_almere">
                    ...
                    @foreach(App\Models\Family::HISTORY_ALMERE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('history_almere', $family->history_almere) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
